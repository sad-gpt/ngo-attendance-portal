...
const PersonLogTab = ({ type }) => {
  ...
  const [exitForm, setExitForm] = useState(null); // { personId, name, exitTime, reason }
  const [editEntry, setEditEntry] = useState(null); // { id, exitTime, returnTime, reason }
  ...
  // Most recent today entry per person (entries come id DESC)
                    ...
                    <td className="px-6 py-3 align-top">
                      {isExitOpen ? (
                        <div className="flex flex-col gap-2 min-w-[320px]">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              disabled={loading}
                              className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                            >
                              Confirm Exit
                            </button>
                            <button
                              onClick={() => setExitForm(null)}
                              className="text-slate-500 dark:text-gray-400 hover:text-slate-700 dark:hover:text-gray-200 text-xs px-2"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div className="flex flex-col gap-2 min-w-[360px]">
                          <div className="flex gap-2 flex-wrap">
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                type="datetime-local"
                                value={editEntry.exitTime}
                                onChange={(e) => setEditEntry({ ...editEntry, exitTime: e.target.value })}
                                className={INPUT_CLS}
                              />
                            </div>
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                type="datetime-local"
                                value={editEntry.returnTime}
                                onChange={(e) => setEditEntry({ ...editEntry, returnTime: e.target.value })}
                                className={INPUT_CLS}
                              />
                            </div>
                            <div className="flex flex-col gap-1 flex-1">
                        ...
                        <button
                          onClick={() => {
                            setEditEntry(null);
                            setExitForm({ personId: person.id, name: person.name, exitTime: nowLocalDT(), reason: "" });
                          }}
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          <button
                            onClick={() => {
                              ...
                              setEditEntry({
                                id: entry.id,
                                exitTime: toLocalDT(entry.exitTime),
                                returnTime: toLocalDT(entry.returnTime),
                                reason: entry.reason || "",
                              });
                            }}
                          ...
                          {!entry.returnTime && (
                            <button
                              onClick={() => handleMarkReturned(entry)}
                              disabled={loading}
                              className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-800/40 px-2.5 py-1 rounded-lg font-medium transition-colors disabled:opacity-50 whitespace-nowrap"
                            >
                              Mark Returned
                            </button>
                          )}
                          <button
                            ...
                            disabled={loading}
                            className="text-xs bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-800/40 px-2.5 py-1 rounded-lg font-medium transition-colors disabled:opacity-50"
                          >
                            Delete
                          </button>
                        </div>
...
const VolunteersTab = () => {
  ...
  const [editRow, setEditRow] = useState(null); // { id, name, reason, arrivalTime, departureTime }
  const [loading, setLoading] = useState(false);
        ...
        <form onSubmit={handleAdd} className="flex gap-3 flex-wrap items-end">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className={`min-w-[160px] ${INPUT_CLS}`}
            />
          </div>
          <div className="flex flex-col gap-1 flex-1">
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
          >
            Log Arrival
          </button>
        </form>
                          ...
                          <div className="flex flex-col gap-2 min-w-[400px]">
                            ...
                            <div className="flex gap-2 flex-wrap">
                              <div className="flex flex-col gap-1">
                                ...
                                <input
                                  type="datetime-local"
                                  value={editRow.arrivalTime}
                                  onChange={(e) => setEditRow({ ...editRow, arrivalTime: e.target.value })}
                                  className={INPUT_CLS}
                                />
                              </div>
                              <div className="flex flex-col gap-1">
                                ...
                                <input
                                  type="datetime-local"
                                  value={editRow.departureTime}
                                  onChange={(e) => setEditRow({ ...editRow, departureTime: e.target.value })}
                                  className={INPUT_CLS}
                                />
                              </div>
                            </div>
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            <button
                              onClick={() => setEditRow({
                                ...
                                name: entry.name,
                                reason: entry.reason || "",
                                arrivalTime: toLocalDT(entry.arrivalTime),
                                departureTime: toLocalDT(entry.departureTime),
                              })}
                              className="text-xs bg-slate-100 dark:bg-gray-600 text-slate-600 dark:text-gray-300 hover:bg-slate-200 dark:hover:bg-gray-500 px-2.5 py-1 rounded-lg font-medium transition-colors"
                            >
                              Edit
                            ...
                            {!entry.departureTime && (
                              <button
                                onClick={() => handleMarkDeparted(entry)}
                                disabled={loading}
                                className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-800/40 px-2.5 py-1 rounded-lg font-medium transition-colors disabled:opacity-50 whitespace-nowrap"
                              >
                                Mark Departed
